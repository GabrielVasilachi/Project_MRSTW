                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {companyDeclarations.map((d, i) => (
                                <tr key={d.id} className="hover:bg-gray-50">
                                    <td className="px-6 py-3 font-mono text-xs text-gray-600">{`INV-2025-${String(100 + i + 1).padStart(4, '0')}`}</td>
                                    ...
                                    <td className="px-6 py-3 text-gray-700">{d.customs_value.toLocaleString()} MDL</td>
                                    <td className="px-6 py-3 text-gray-700">{d.vat.toLocaleString()} MDL</td>
                                    <td className="px-6 py-3 text-gray-700">{d.customs_duty.toLocaleString()} MDL</td>
                                    <td className="px-6 py-3 font-semibold text-gray-900">{d.total_taxes.toLocaleString()} MDL</td>
                        ...
                        <tfoot className="border-t-2 border-gray-200 bg-gray-50 text-sm font-semibold text-gray-800">
                            <tr>
                                ...
                                <td className="px-6 py-3">{companyDeclarations.reduce((s, d) => s + d.customs_value, 0).toLocaleString()} MDL</td>
                                <td className="px-6 py-3">{companyDeclarations.reduce((s, d) => s + d.vat, 0).toLocaleString()} MDL</td>
                                <td className="px-6 py-3">{companyDeclarations.reduce((s, d) => s + d.customs_duty, 0).toLocaleString()} MDL</td>
                                <td className="px-6 py-3">{companyDeclarations.reduce((s, d) => s + d.total_taxes, 0).toLocaleString()} MDL</td>
                                <td /><td />
                            </tr>
                        </tfoot>
                    </table>
